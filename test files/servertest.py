...
def show_panel(conn, hidden_word, lives, wrong_letters):
...
def game_loop(conn): #receives conn from handle_client
    ...
    while True:
        ...
        target = ["-" if c == " " else c for c in chosen_word.lower()]

        #main logic loop
        while lives > 0 and hidden_word != target: #list(chosen_word.lower()):

            #make sure that only one letter is chosen on each round
            while True:
                show_panel(conn, hidden_word, lives, wrong_letters)
